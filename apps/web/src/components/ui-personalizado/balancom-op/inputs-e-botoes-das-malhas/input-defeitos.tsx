...
type InputDefeitosProps = {
  idBm: string;
  ref: string;
  defeitosStock: number;
};

const InputDefeitos = ({ idBm, ref, defeitosStock }: InputDefeitosProps) => {
  const trpc = useTRPC();
  ...
  const { mutate: insereDefeito, isPending } = useMutation(
    trpc.qualidade_balancom_op.patchBmDefeitos.mutationOptions({
            ...
            return {
              ...
              BmMalhas: old.BmMalhas.map((item) =>
                item.idBm === idBm && item.ref === ref
                  ? { ...item, defeitosStock: valor.defeitosStock as number }
                  : item
              ),
            };
          }
